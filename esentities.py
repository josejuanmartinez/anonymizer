...
class ESEntities(Enum):
    ...
    def get_regex(cls, ent):
        if ent == ESEntities.PHONES:
            ...
            return re.compile(r"(Número|número|Num\.|Núm\.|num\.|núm.|N\.|n\.|Nº|nº)[\s\n]*:?[\s\n]*[0-9]*")
        elif ent == ESEntities.NAME:
            return re.compile(r"(Nombre|nombre|Apellidos|apellidos)[\s\n]*:?[\s\n]*[a-zA-Z, ]*")
        elif ent == ESEntities.SEX:
            return re.compile(r"(Sexo|sexo)[\s\n]*:?[\s\n]*[a-zA-Z]*")
        elif ent == ESEntities.AGE:
            return re.compile(r"(Edad|edad)[\s\n]*:?[\s\n]*[0-9]*")